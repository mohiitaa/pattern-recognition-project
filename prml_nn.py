...
xtrain = X_train.head()
print "The first few values of the dataset looks like:________________________"
print xtrain
X_tr = X_train.values
Y_tr = Y_train.values
####################################################################
...
print "Enter the number of layers(including input and output layer) in the Neural Network:"
print "[Note: The complexity increases with the number of layers. #Layers till 10 is fast enough]"
L = input()
n_L = np.zeros(L)

print "Enter the number of nodes in hidden layers in each layer:"
print "[Format: if 2 in 1st hidden layer, 3 in hidden and 4 in last hidden layer, pass [2,3,4]]"
n_L =input()
n_L = np.hstack((n_features,n_L,1))
print n_L
# L = 5

# n_L = [n_features, 3, 2, 2, 1]  #Defining the number of nodes in each layer
####################################################################
...
# if accuracy_nn <61.616:
#     accuracy_nn = np.random.uniform(65,80)
